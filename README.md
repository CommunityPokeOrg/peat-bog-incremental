...
- **Achievements**: 25 to unlock, each granting +1% to all production. The **Docket** tab orders the trial milestones and highlights the next filing.
- **Charter**: spend banked Bog Cores on the 12-term Drainage Charter. Signed terms persist through every draining, but spent cores stop contributing their +5% production bonus.
...
- **Saving**: IndexedDB via `idb` autosaves every 15 s and on tab close, with a localStorage fallback. Earn offline progress at 50% rate, capped at 8 hours; Standing Order Charter terms can raise the offline rate to 100%. Export/import saves as base64 from the Settings tab. Rotating field notes appear in the footer.
