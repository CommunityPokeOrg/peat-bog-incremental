...
export const CHARTER_BRANCHES: Record<CharterBranch, { name: string; blurb: string }> = {
  ...
  roots: { name: 'Retention', blurb: 'What the firm remembers between restructures.' },
  kindling: { name: 'Bandwidth', blurb: 'Latency, leaks, and the racks that run on both.' },
  filing: { name: 'Filing', blurb: 'The court keeps working while you are away.' },
  hands: { name: 'Billables', blurb: 'Clicks, fieldwork, and the practical business of billing.' },
  engines: { name: 'Engines', blurb: 'Passive production, cooling, and the machinery underneath the docket.' },
  lamps: { name: 'Night Desk', blurb: 'The screens that stay on after the associates go home.' },
  stills: { name: 'Clearing', blurb: 'Transmutation, refinement, and useful residue.' },
  keepers: { name: 'Partners', blurb: 'The people and filings the firm refuses to forget.' },
};

const BASE_CHARTER: CharterNodeDef[] = [
  { id: CHARTER_ROOT_ID, name: "Magistrate's Seal", emoji: '💠', description: 'The wax that makes the Restructuring Charter binding. Every branch grows from here.', branch: 'seal', cost: 1, effects: [{ kind: 'multiplier', target: 'all', factor: 1.05 }] },
  { id: 'roots-1', name: 'Institutional Memory', emoji: '🌿', description: 'The associates know where the good discovery is now.', branch: 'roots', cost: 1, requires: CHARTER_ROOT_ID, effects: [{ kind: 'multiplier', target: 'broth', factor: 1.25 }] },
  { id: 'roots-2', name: 'Legacy Positions', emoji: '🪵', description: 'Screens and discovery hold the book together.', branch: 'roots', cost: 2, requires: 'roots-1', effects: [{ kind: 'multiplier', target: 'peat', factor: 1.5 }, { kind: 'multiplier', target: 'sphagnum', factor: 1.5 }] },
  { id: 'roots-3', name: 'Head Start', emoji: '🪣', description: 'A restructured firm refills faster than the paperwork says.', branch: 'roots', cost: 2, requires: 'roots-2', effects: [{ kind: 'startingBroth', amount: 10_000 }] },
  { id: 'roots-4', name: 'Docket Memory', emoji: '🧠', description: 'The firm keeps a copy of every useful habit.', branch: 'roots', cost: 4, requires: 'roots-3', effects: [{ kind: 'multiplier', target: 'all', factor: 1.5 }] },
  { id: 'roots-3b', name: 'Privilege Ledger', emoji: '🌾', description: 'The screens keep their own accounts, and they favour us.', branch: 'roots', cost: 3, requires: 'roots-2', effects: [{ kind: 'multiplier', target: 'sphagnum', factor: 2 }] },
  { id: 'kindling-1', name: 'Warm Racks', emoji: '🔥', description: 'The racks waste less latency into the courthouse.', branch: 'kindling', cost: 1, requires: CHARTER_ROOT_ID, effects: [{ kind: 'heat', factor: 0.9 }] },
  { id: 'kindling-2', name: 'Leak Bloom', emoji: '💨', description: 'The memos rise on schedule and pay their share.', branch: 'kindling', cost: 2, requires: 'kindling-1', effects: [{ kind: 'multiplier', target: 'methane', factor: 1.5 }] },
  ...
  { id: 'kindling-3b', name: 'Flare Discipline', emoji: '🕯️', description: 'The burn notices fire only what the racks cannot use.', branch: 'kindling', cost: 3, requires: 'kindling-2', effects: [{ kind: 'heat', factor: 0.85 }] },
  ...
  { id: 'filing-3', name: 'Pulley Dividend', emoji: '🔩', description: 'Every click carries a little more tokens into evidence.', branch: 'filing', cost: 2, requires: 'filing-2', effects: [{ kind: 'clickBrothFraction', fraction: 0.02 }] },
  { id: 'filing-4', name: "Reino's Precedent", emoji: '⚖️', description: 'The Magistrate allows a larger precedent award.', branch: 'filing', cost: 5, requires: 'filing-3', effects: [{ kind: 'coreGain', factor: 1.5 }] },
...
export const CHARTER: CharterNodeDef[] = [...BASE_CHARTER, ...WING_NODES];
