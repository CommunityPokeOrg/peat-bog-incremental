...
const BOOST_NAME_SUFFIXES = ['Second Shift', 'Rail Spur', 'Union Contract', "Magistrate's Seal"];
export const BUILDINGS: BuildingDef[] = BASE_BUILDINGS.map((building) => {
  ...
  return {
    ...building,
    costScale,
    boostNames: BOOST_NAME_SUFFIXES.map((suffix) => `${building.name} ${suffix}`) as [string, string, string, string],
  };
});
